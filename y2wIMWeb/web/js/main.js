...
var y2w = {
    ...
    addEvent: function () {
        ...
        this.$chatContent.delegate('a','click',this.openUrl);
    ...
    beginChat:function(){
        var account = $.trim(this.$addFriendBox.find(".j-uid").html().toLowerCase());
        this.hideAddFriend();
    ...
    openUrl:function(){
        ...
        if(url)
            y2w.openChatBox(url,'url');//.showInfoBox(user);
    },
    ...
    syncMessages: function(userConversation, force, cb){
        ...
        userConversation.syncMessages(force, function(err, messages){
            ...
            currentUser.sessions.get(userConversation.targetId, userConversation.type, function(err, session) {
                ...
                if(messages && session.id==currentUser.currentSession.id) {
    ...
    showMsg: function(msg){
        ...
        if(!$dom || $dom.length == 0){
            ...
            try{
                var prev=$dom.prev();
                if(prev[0].tagName.toUpperCase() =="P" && prev.attr("class").indexOf("invisible")>=0 )
                    prev.remove();
            }
            catch(ex){
    ...
    openChatBox: function (account,scene) {
        ...
        if(scene=="p2p"){
            ...
            currentUser.sessions.get(info.id, scene, function(err, session){
                ...
                //that.changeCurrentSession(session,scene,account);
        ...
        //this.$rightPanel.find('.chat-box').addClass("hide");
        this.$rightPanel.find('.browser').addClass("hide");
        //设置聊天面板
        if (scene === 'p2p') {
            var text;
            var avatarUrl;
    ...
    //changeCurrentSession:function(session,scene,account){
    //    currentUser.currentSession = session;
    //    this.$chatEditor.data({
    //        to: account,
    //        type: scene
    ...
    relogin:function(cb){
        var that=this;
        currentUser.relogin(function(error,token){
            if(error)
                return that.logout();
